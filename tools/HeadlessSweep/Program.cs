...
if (args.Length == 1 && string.Equals(args[0], "refit-determinism", StringComparison.Ordinal))
{
    return RefitDeterminismProbe.Run(FindRepositoryRoot());
}

if (args.Length == 1 && string.Equals(args[0], "seal-determinism", StringComparison.Ordinal))
{
    return SealDeterminismProbe.Run(FindRepositoryRoot());
}
...
if (args.Length >= 1 && string.Equals(args[0], "credible-deck-matchup", StringComparison.Ordinal))
{
    return CredibleDeckMatchupRunner.Run(FindRepositoryRoot(), args.Skip(1).ToArray());
}
...
Console.Error.WriteLine(
    ...
    + "| refit-determinism | seal-determinism "
