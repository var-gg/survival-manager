...
public sealed partial class GameSessionState
{
    ...
    /// BattleScreenController.cs:1092-1111의 인라인 합성과 동일 계약(씬은 후속에서 이 메서드를 채택).
    /// </summary>
    public bool TryBuildSelectedBattleState(out BattleState state, out ResolvedEncounterContext encounter, out string error)
    {
        state = null!;
        encounter = null!;

        var allySnapshot = BuildBattleLoadoutSnapshot();
    ...
    /// 헤드리스 플레이가 진짜로 싸운다. 합성 경로는 FirstPlayableBalanceRunner.cs:752-759에서 검증된 것과 동일.
    /// </summary>
    public bool TryResolveSelectedBattleNodeViaSimulation(out BattleResult result, out string error)
    {
        ...
        if (!TryResolveCurrentEncounter(out var encounter, out error))
        {
            return false;
        }
