...
public static class LocalizationFoundationBootstrap
{
    ...
    private static readonly Dictionary<string, (string ko, string en, bool smart)> SharedEntries = new(StringComparer.Ordinal)
    {
        ...
        ["ui.common.start_screen.status"] = ("로컬 플레이어블 실행을 시작합니다.", "Start the local playable run.", false),
        ["ui.common.start_screen.hint"] = ("이번 빌드는 단일 local/authored loop와 JSON 저장만 지원합니다.", "This build exposes one local/authored loop with JSON save.", false),
