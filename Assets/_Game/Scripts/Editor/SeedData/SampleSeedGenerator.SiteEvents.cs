...
public static partial class SampleSeedGenerator
{
    ...
    private static void CreateSiteEventContent()
    {
        CreateSiteEvent(
            ...
            Choice("rescue", "site_event_choice_rescue", "의무병을 영입 후보로 확보한다", "Secure a medic recruit offer",
                Outcome(OutcomeKind.GrantRecruitOffer, "priest", "trait_frail")),
            Choice("take_supplies", "site_event_choice_take_supplies", "응급 붕대와 갑옷을 얻는다", "Take field dressings and armor",
                Outcome(OutcomeKind.GrantConsumable, "consumable_field_dressing", amount: 2),
                Outcome(OutcomeKind.GrantItem, "item_layered_armor")),
            Choice("burn_it", "site_event_choice_burn_it", "메아리 6을 얻고 구호소를 태운다", "Gain 6 Echo and burn the station",
        ...
        CreateSiteEvent(
            ...
            Choice("recover_scout", "site_event_choice_recover_scout", "사냥꾼 활을 회수하고 매복 정예로 향한다", "Recover a hunter bow and route to the elite ambush",
                Outcome(OutcomeKind.GrantItem, "item_hunter_bow"),
                Outcome(OutcomeKind.RouteToNode, "node_wolfpine_ambush_elite")),
            Choice("move_on", "site_event_choice_move_on", "아무 변화 없이 표준 경로로 전진한다", "Advance on the standard route",
        ...
        CreateSiteEvent(
            ...
            Choice("trade", "site_event_choice_trade", "보급품과 소모품을 받고 철수 보너스 3을 확보한다", "Trade for supplies, a consumable, and 3 extract bonus",
            ...
            Choice("claim_bounty", "site_event_choice_claim_bounty", "메아리 8을 현상금으로 받는다", "Claim an 8 Echo bounty",
                Outcome(OutcomeKind.GrantEcho, amount: 8)),
            Choice("seize", "site_event_choice_seize", "장비 둘을 빼앗고 전열 한 명이 전상당한다", "Seize two items and wound one frontliner",
        ...
        CreateSiteEvent(
            ...
            Choice("salvage", "site_event_choice_salvage", "철수 보너스 10을 얻고 변형 우두머리로 향한다", "Gain 10 extract bonus and route to the boss variant",
                Outcome(OutcomeKind.ExtractBonus, amount: 10),
                Outcome(OutcomeKind.RouteToNode, "node_sunken_boss_variant")),
            Choice("leave", "site_event_choice_leave", "표준 우두머리 경로로 향한다", "Route to the standard boss",
                Outcome(OutcomeKind.RouteToNode, "node_sunken_boss_standard")),
            Choice("break_seal", "site_event_choice_break_seal", "성물을 얻고 전열 한 명이 전상당한다", "Take the relic and wound one frontliner",
        ...
        CreateSiteEvent(
            ...
            Choice("assault", "site_event_choice_assault", "추가 전투로 돌입한다", "Route into the bonus fight",
                Outcome(OutcomeKind.RouteToNode, "node_hostage_bonus_fight")),
            Choice("ransom", "site_event_choice_ransom", "메아리 10을 지불하고 쉬운 심층 경로로 간다", "Pay 10 Echo and take the easier deep route",
                Outcome(OutcomeKind.GrantEcho, amount: -10),
                Outcome(OutcomeKind.RouteToNode, "node_hostage_easy_deep")),
            Choice("ignore", "site_event_choice_ignore", "아무 변화 없이 떠난다", "Leave without changing anything"));
        ...
        CreateSiteEvent(
            ...
            Choice("offer_echo", "site_event_choice_offer_echo", "메아리 5를 얻는다", "Gain 5 Echo",
                Outcome(OutcomeKind.GrantEcho, amount: 5)),
            Choice("study_names", "site_event_choice_study_names", "배치 서수가 가장 낮은 영웅이 경험치 25를 얻는다", "The lowest deployed hero gains 25 experience",
                Outcome(OutcomeKind.GrantExp, amount: 25, targetRule: OutcomeTargetRule.LowestDeployIndex)),
            Choice("lay_to_rest", "site_event_choice_lay_to_rest", "전상 한 명을 치료한다", "Cure one wounded hero",
        ...
        LocalizationFoundationBootstrap.SyncSiteEventChoiceSurfaceEntries();
    }
    ...
    private static SiteEventChoiceSeed Choice(
        string id,
        string iconId,
        string koLabel,
        string enLabel,
        params SiteEventOutcomeDefinition[] outcomes) =>
        new(id, iconId, koLabel, enLabel, outcomes);
