...
public static class CombatSandboxAuthoringAssetUtility
{
    ...
    public static void EnsureStarterLibrary()
    {
        ...
        var observerSmoke = GetOrCreateAsset<TeamLoadoutPresetAsset>(
            ...
            asset =>
            {
                ...
                asset.Source.Members = new List<CombatSandboxPresetMemberSpec>
                {
                    new() { MemberId = "enemy_guardian", DisplayName = "Enemy Guardian", SourceKind = SandboxUnitSourceKind.Character, CharacterId = "guardian", Anchor = SM.Combat.Model.DeploymentAnchorId.FrontTop },
                    new() { MemberId = "enemy_raider", DisplayName = "Enemy Raider", SourceKind = SandboxUnitSourceKind.Character, CharacterId = "raider", Anchor = SM.Combat.Model.DeploymentAnchorId.FrontBottom },
                    new() { MemberId = "enemy_hunter", DisplayName = "Enemy Hunter", SourceKind = SandboxUnitSourceKind.Character, CharacterId = "hunter", Anchor = SM.Combat.Model.DeploymentAnchorId.BackTop },
                    new() { MemberId = "enemy_hexer", DisplayName = "Enemy Hexer", SourceKind = SandboxUnitSourceKind.Character, CharacterId = "hexer", Anchor = SM.Combat.Model.DeploymentAnchorId.BackBottom },
                };
        ...
        CreateScenario("anti_burst_regression", "Anti Burst Regression", endgameFortress, endgameGlassCannon, regressionBatch, true, "regression", "anti_burst");
        ...
        params string[] tags)
    {
        ...
        if (AssetDatabase.LoadAssetAtPath<CombatSandboxScenarioAsset>(path) != null)
        {
            return;
        }

        GetOrCreateAsset<CombatSandboxScenarioAsset>(
            path,
            created =>
            {
                created.ScenarioId = scenarioId;
                created.DisplayName = displayName;
                created.IsFavorite = favorite;
                created.LeftTeam = leftTeam;
                created.RightTeam = rightTeam;
                created.ExecutionPreset = execution;
                created.Tags = tags.ToList();
                created.ExpectedOutcome = favorite
                    ? "Primary starter scenario."
                    : "Regression / matchup authoring baseline.";
                created.Notes = "Generated by CombatSandboxAuthoringAssetUtility.";
            });
    }
