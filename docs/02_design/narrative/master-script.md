...
`dialogue-event-schema.md`의 presentation grade 규격을 따른다. 이 문서와 schema가 충돌하면 schema가 우선.

| 유형 | 용도 | 라인 수 |
|---|---|---|
| `dialogue-scene` | 핵심 스토리 장면. Narrator + 복수 화자. SceneTurn 4슬롯 필수. | 8~14 |
| `dialogue-overlay` | 전투 전후 bark, 짧은 반응, 전투 결과 반응, town 반응. | 2~6 |
| `story-card` | 사이트 extract, 챕터 intro/outro, 대형 reveal 여운. | 1~3문단 |

## 대사 생성 규칙

`narrative-pacing-formula.md`의 "장면 문법 공식"과 "대사 생성 지침"을 반드시 적용한다. 핵심 규칙 요약:

1. 모든 `dialogue-scene`은 SceneTurn 4슬롯(hook→resistance→proof→changed) 준수
2. 설정 핵심명사(Heartforge, 격자, 정화, 봉인, 순환, 질서, 공명, 기억) 직접 언급은 한 scene 내 3회 이하
3. 모든 `dialogue-scene`에 구체적 사물(ObjectAnchor) 1개 이상 필수
4. narrator는 감정 해설 금지 — 관찰 가능한 동작/사물/환경만 묘사
5. 장면 첫 줄은 요약 금지, 마지막 줄은 결론문 금지
6. 리드별 음성 제한은 하단 "화자 어조 요약" 참조

## 화자 어조 요약

| 화자 | 어체 | 핵심 특징 | 허용 어휘군 | 금지 패턴 | 감정 붕괴 시 |
|---|---|---|---|---|---|
| Dawn Priest | 합니다/습니다 | 젊은 여성 사제. ch1-2 교리적 확신, ch3+ 문장이 끊기고 질문이 나옴, ch5 조용한 결의 | 사물/의식어: 묵주, 성반, 인장, 성물함, 재, 기도문 | 범용 결의문("끝내야 합니다"), reveal 결론 직접 선언, 추상명사 나열 | 문장이 부서짐. self-correction, 끊김, 말의 실패. "...아닙니다. 아니, 그게..." |
| Pack Raider | 해/다 | 직설, 감각적, 빈정거림. 냄새/소리/질감으로 사고 | 감각어: 바람, 흙, 냄새, 이빨, 피, 발톱, 뿌리 | 냄새/바람 은유 남용(사이트당 동일 감각 은유 2회 초과), 같은 비유 반복 | 은유가 사라짐. 직설적이고 짧아짐. 감각 대신 사실만 말함 |
| Grave Hexer | 했지/이었네 | 건조한 유머가 방어기제. 수백 년을 최근 취급. 절대 시적이지 않음 | 기억 관리어: 분류, 퇴적, 잔향, 오기, 누락, 보관, 묶기 | 순수 gag(상황 무게를 해제하는 웃음), 유머 연속 2회, MCU식 재치 | 유머가 사라지고 건조함만 남음. 전문가가 자기 도구를 잃은 순간 |
| Echo Savant | 한다/이다 | 최소 단어. 신체 감각. 1800년 잠에서 깸. 혼란스러움, 권위적이지 않음 | 기술/감각어: 누수, 잔향, 파장, 공명, 진동, 열기 | 기술설명 연쇄(한 scene 내 기술 은유 2회 초과), lore terminal 역할 | 인간적 기억 파편이 누출. 촉감, 열기, 오래된 목소리. "...따뜻했다. 그것만 기억한다." |
| Aldric Sternholt | 했소/이오 | 고풍 격식. 차분하고 합리적. 자기가 옳다고 확신. 악당이 아니라 설계자 | 관리/유지어: 통제, 유지, 설계, 작동, 균형, 안정 | 분노 독백, 광기 표현, 과장된 악당 선언 | 변하지 않음. 끝까지 calm, clerical. 이것이 공포의 원천 |
| Narrator | — | 인용 부호 없음. 현재형. 감각 묘사 1~2문장 | 물리적 관찰: 동작, 사물, 환경, 소리, 빛 | 감정 해설("무겁다/무섭다"), 의미 선포("가장 ~한"), 독자 대신 해석 | — |
...
### `story_card_extract_ashen_gate` — 재의 문 추출

> **연출**: story-card

**재의 문을 지나다**

재가 바람에 실려 등 뒤로 흘러간다. 국경문의 철 파편이 점점 작아지고, Pack Raider의 주머니에서 파편이 미세하게 진동한다. 아래로. 더 아래로.

원정대는 첫 번째 답을 얻었다. 문은 밖에서 부서진 것이 아니었다. 하지만 그 답이 열어둔 질문이 더 무겁다 — 안에서 무엇이 밀어냈는가.

---

### `story_card_extract_wolfpine_trail` — 늑대소나무 길 추출

> **연출**: story-card

**소나무 사이를 떠나다**

Pack Raider가 마지막 토템 앞에서 멈춘다. 손을 댄다. 진동은 여전히 거기 있다. 이번에는 응답하지 않는다.

형제의 파편이 주머니 안에서 차갑게 식어간다. 처음으로, 같은 방향을 보고 걷는 두 사람. 아직 동맹이라 부르기엔 이르지만, 적이라 부르기엔 이미 늦었다.
...
| 3 | Dawn Priest | weary | "제가 기도를 올리던 성반이... 이 기둥과 같은 돌입니다. 같은 양식으로 금이 가고 있어요." |
...
### `story_card_extract_tithe_road` — 십일조 길 추출

> **연출**: story-card

**가라앉은 보루를 등지다**

요새 최하층에서 물이 천천히 차오른다. Dawn Priest의 발밑에 사제 인장이 잠겨 있다. 꺼내지 않는다.

그녀가 80년간 떠받들던 이름이 일지 위에 적혀 있다. A. Sternholt. 기도를 설계한 자. 방벽을 설계한 자. 모든 것을 설계한 자. 이름이 생긴 적은 아직 보이지 않지만, 이제 그가 어딘가에 있다는 것만은 안다.
...
| 2 | Dawn Priest | shock | "우리가 문을 부수려 했고... 저들은 마지막 빗장을 붙들고 있었다는 겁니까?" |
...
### `story_card_extract_bone_orchard` — 뼈 과수원 추출

> **연출**: story-card

**세계가 뒤집힌 뒤**

유골 과수원의 인광이 서서히 꺼진다. 네 종족이 같은 물을 마시던 기억이 흩어지고, 1800년의 잠에서 깨어난 자가 서 있다. 격자가 40% 이하. 남은 시간이 많지 않다.

원정의 목적이 바뀌었다. 조사가 아니다. 수리가 아니다. 순환의 종결이다. 그리고 그 대가가 무엇인지, 아직 아무도 모른다.
...
| 4 | Dawn Priest | grim | "당신이 봉인을 고르면 200년 뒤에 당신 새끼들이 이 자리에 서요. 당신 이름으로 또 전쟁을 시작하고." |
| 5 | Pack Raider | bitter | "...그래도 지금 여기서 눈앞의 사람을 묻을 순 없어. 200년은 아직 태어나지도 않았잖아." |
...
| 13 | Narrator | — | 아무도 움직이지 않는다. 유리 가지가 바람에 부딪혀 울린다. Pack Raider의 토템이 손에서 한 번 미끄러진다. |
...
| 9 | Dawn Priest | solemn | "이걸 멈추지 못하면 제 기도는 또 누군가를 태웁니다." |
...
| 11 | Narrator | — | Dawn Priest의 손이 멈춘다. Pack Raider의 코가 움찔한다. Grave Hexer가 농담을 꺼내려다 — 입을 다문다. |
...
### `story_card_extract_starved_menagerie` — 굶주린 동물원 추출

> **연출**: story-card

**대가의 윤곽**

유리 숲의 결정이 바람에 울린다. 속삭임은 잠시 멈췄지만, 아래에서 여전히 맥동이 올라온다.

정화. 봉인. 파괴. 세 갈래 앞에서 네 사람이 서로 다른 방향을 보았다. 합의는 없다. 하지만 한 가지는 확실하다 — 어떤 길을 고르든, 되돌아올 수 없는 것을 놓아야 한다. 한 사람은 이미 자원했다.
...
### `story_card_extract_worldscar_depths` — Worldscar 심부 추출 (캠페인 최종)

> **연출**: story-card

**세 사람이 올라온다**

Worldscar의 좁은 틈 사이로 하늘이 보인다. 재가 아니라 빛이 내려온다.

Dawn Priest의 손에 사제 인장이 없다. Aldric의 일지가 있다. Pack Raider의 주머니에서 파편이 더 이상 진동하지 않는다. Grave Hexer는 고개를 돌려 뒤를 본다 — 무엇을 찾는지 본인도 모른다.

아래에서 프리즘 보라빛이 맥동한다. 조용히. 규칙적으로. 누군가가 거기 남아 있다.

지치고, 상처 입고, 줄어들었다. 하지만 서 있다.
