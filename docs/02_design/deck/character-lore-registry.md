...
자신이 평생 섬긴 교단의 기반이 약탈한 유물과 은폐 위에 세워졌다는 사실을 감당해야 한다. 신앙을 버리면 정체성이 무너지고, 유지하면 거짓의 공범이 된다. 캠페인 종료 시점에 그녀는 균열을 인지하지만 완전한 해결에는 이르지 못하며, 새로운 신앙의 형태를 모색하는 단계에서 이야기가 열린 채 남는다.
...
그녀가 지닌 기억은 과거의 증거이자 무기다. 공개하면 모든 세력의 정당성이 흔들리고, 은폐하면 순환이 반복된다. 기억의 빚을 청산하려면 진실을 드러내야 하지만, 그것이 취약한 동맹을 파괴할 수도 있다. 캠페인 종료 시 그녀는 기억의 빚을 인지하지만, 완전한 청산은 후속 이야기의 과제로 남는다.
