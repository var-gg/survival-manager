...
이 문서는 20명/4종족 확장 목표를 설명하는 roadmap이다. 현재 실행 source-of-truth는 `CharacterDefinition` 16개, `RaceDefinition` 3개, `UnitArchetypeDefinition` 16개로 닫혀 있다.
...
- Relicborn runtime 승격 조건: `race_relicborn`, Relicborn core archetype 4종, matching `CharacterDefinition`, unlock/reward route가 같은 작업 단위에서 닫혀야 한다.
- `hero_mirror_cantor`는 현재 실행 safe target에 specialist로 존재하지만, 이것만으로 4번째 종족 runtime closure가 끝났다고 보지 않는다.
