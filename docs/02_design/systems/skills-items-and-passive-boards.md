...
- item / affix는 hero별 local specialization을 만든다.
- passive board는 hero별 장기 방향성을 만들고, selection은 prerequisite / exclusion / cap validator를 통과해야 한다.
- permanent augment는 squad blueprint별 `1-slot thesis`를 만든다.
- temporary augment는 run overlay를 바꾸고, 첫 선택은 다음 Town에서 permanent candidate unlock으로 이어질 수 있다.
- passive node와 augment는 전투 직전에 modifier package와 compile tag로 압축된다.
...
- passive board: 중형 보드 + keystone 중심, hero당 active `5`, keystone `1`
- permanent augment: blueprint당 equip `1`
- Town build-management lane: retrain / refit / passive board / permanent equip를 정식 action row로 노출
...
- permanent unlock grammar는 `docs/02_design/meta/permanent-augment-progression.md`가 소유한다.
