...
| H100 player-visible observation과 정책 선택, BT1 의도 trace, E06 preview-grounded 적응, E07 Town agency·Refit/Seal | `SM.HeadlessPolicies` + `SM.HeadlessMetrics` trace + sealed LLM `SM.SealedLlmBridge` codec + 기존 `SM.Editor` projection adapter | `test-batch-fast`, 정책별 `h100-metrics.ps1`, `h100-policy-witness.ps1`, `h100-intent-trace.ps1`, `h100-preview-policy.ps1`, `h100-seal-policy-measurement.ps1` | 정책·trace schema와 codec 함수는 engine-free; 실제 session projection/corpus, E03 계약 주입, Town session API 실행, paired acceptance는 editor-required |
