...
Town runner는 보상 정산 뒤 `ReturnToTownAfterReward()`가 완료된 사이트 사이에만 실행한다. 순서는 영입→노드→Refit이며 각 행동 뒤 현재 session에서 observation과 decision seed를 다시 만든다. 영입은 필요한 count tag와 직접 identity/substitution을 lexicographic으로 우선하고, 노드는 관련 target의 합법 선행 chain부터 채운다. Refit은 새 결과를 예측하지 않고 missing-affix intent가 있을 때 현재 합법·지불 가능한 item action anchor를 모두 비교한다. 현재 품질과 exact quote만으로 양의 Seal 순가치가 성립하는 item을 먼저 고르고, 그런 item이 없으면 평균 roll quality가 가장 낮아 plain Refit 개선 여지가 큰 item을 고른다. 동률은 affix 수 내림차순, item ID, item instance ID, slot index의 ordinal 순서로 닫는다.

`HeadlessRefitDecision.AffixSlotIndex`는 plain Refit이 개선할 affix를 뜻하지 않는다. production `RefitService.RefitNextEffective`는 기존 affix identity를 유지한 채 모든 magnitude를 다시 굴리며, observation builder가 `CanRefit=true`로 노출하는 slot 0은 기존 action menu의 합법성 anchor다. 따라서 Seal 정책은 선택된 item의 `AffixSlotIndex`를 그대로 유지하고 현재 affix별 roll quality 가운데 가장 낮은 affix를 reroll target으로 삼는다. 다른 affix 중 quality 0.70 이상을 높은 quality 순으로 lock 후보에 넣고, 보존가치 합에서 plain Refit 대비 Seal 추가비용의 현재 wallet 비율을 뺀 값이 0.01보다 클 때만 lock한다. 동률은 비용이 낮은 선택, 그 다음 ordinal lock signature 순으로 깬다.
...
no-Seal substrate에서 실제 적용된 Refit decision의 candidate와 slot을 action descriptor에서 확정한 뒤 Gold/Echo wallet, affix별 `RefitRollQuality`, authored magnitude range, Seal quote, candidate-selection bias를 매 window 원시 행으로 남긴다. candidate를 별도 ordering으로 재구성하지 않으므로 production selector가 바뀌어도 측정기는 과거 ordering을 강제하지 않는다. 같은 행에는 전체 visible inventory의 모든 item ID/instance, affix 수와 품질, 합법성·지불 가능성, exact Seal quote를 함께 남겨 candidate가 비었을 때 실제 affix-barren인지 legality/affordability로 막혔는지 직접 구분한다. primary 분포는 고정 10-bin histogram과 `p0/p10/p25/p50/p75/p90/p100` 선형 보간 quantile이다. 기본 `full` 모드는 threshold `0.50/0.70/0.85`, minimum net-value floor `0.00/0.01/0.05`, preservation baseline `0.40/0.50/0.60`의 27개 policy-only calibration을 같은 32개 campaign에 paired 실행한다. 진단용 `census` 모드는 no-Seal raw census만, 회귀용 `shipped` 모드는 출고 상수 `(0.70, 0.01, 0.50)` arm만 실행하며 seed·관측량·정책식은 바꾸지 않는다. adapter는 일반 Refit target과 non-Refit 결정을 production `ConceptCommitPolicy`에 위임하고 Seal lock set만 바꾸며, shipped policy constant와 authored content는 수정하지 않는다.
