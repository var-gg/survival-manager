...
- `SM/Internal/Content/Ensure Sample Content`와 `SM/Internal/Content/Generate Sample Content`는 explicit preflight/write lane이다.
...
- `SM/Internal/Content/Migrate Legacy Sample Content`는 legacy root가 있을 때만 `AssetDatabase.MoveAsset` 기반 이동을 시도한다.
...
- 1차 recovery action은 `SM/Internal/Content/Ensure Sample Content`다.
- committed floor authoring이 깨졌거나 launch-floor coverage가 모자라면 `SM/Internal/Content/Generate Sample Content`를 명시적으로 실행해 repair한다.
