...
- `Boot`만 기존 최소 uGUI canvas를 유지하고 session realm 선택 화면을 가진다.
...
- presenter/view는 `SaveProfile` 내부 컬렉션 shape를 직접 읽는 새 의존을 늘리지 않는다. session/profile summary는 view DTO를 우선한다.
...
- Boot는 realm 선택용 uGUI button/text contract를 유지한다.
- Town은 current realm 기반 local progression 요약과 action만 노출한다.
- Reward는 local reward 흐름만 노출하고 official upload/status UI는 두지 않는다.
