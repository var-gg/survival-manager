...
| asmdef | 위치 | 용도 |
| --- | --- | --- |
| `SM.Tests.FastUnit` | `Assets/Tests/EditMode/FastUnit/` | FastUnit editor-free/resource-free/authored-object-free lane |
| `SM.Tests.EditMode` | `Assets/Tests/EditMode/` | category 기반 BatchOnly/EditMode 테스트와 editor validation 테스트 |
...
`SM.Tests.FastUnit`은 Unity Test Framework의 EditMode 실행 조건 때문에 `Editor` platform target을 유지하지만 `SM.Editor`와 editor-only package를 참조하지 않는다. 현재 `BatchOnly` 테스트 일부는 `SM.Tests.EditMode` 루트에 category 기반으로 남아 있다. `SM.Tests.EditMode.Integration`은 장기 이동 대상이지, 현재 모든 BatchOnly 테스트의 유일한 위치는 아니다.
...
`SM.Unity`, content conversion/bootstrap, `RuntimeCombatContentLookup`, `GameSessionRuntimeBootstrapProvider`, `NarrativeRuntimeBootstrap.LoadFromResources()`, public `GameSessionState` constructor, authored `ScriptableObject`, UI/controller/scene/prefab 검증은 FastUnit closure 내부가 아니다. 이들은 `BatchOnly`, PlayMode, editor-required 검증으로 라우팅한다.
...
| `ManualLoopD` | 장시간 telemetry/balance review 전용 수동 lane | X | O |

기본 원칙은 모든 EditMode test class가 `FastUnit` 또는 `BatchOnly` 중 하나를 명시하는 것이다. `FastUnit` class는 `Assets/Tests/EditMode/FastUnit/**`에만 둔다. `ManualLoopD`는 장시간 Loop D balance/telemetry 검증용 예외 lane이며, default fast loop나 일반 BatchOnly closure의 대체물이 아니다. symmetric mirror 4v4 timeout/draw policy probe도 FastUnit deterministic end oracle과 분리해 `ManualLoopD`에서 추적한다.

### FastUnit 테스트 작성 규칙

- `FakeCombatContentLookup`을 사용한다 (`ICombatContentLookup` 구현, asset 로딩 없음).
- `Assets/Tests/EditMode/FastUnit/**` 밖에 두지 않는다.
- `new RuntimeCombatContentLookup()`를 직접 호출하지 않는다.
- session test는 `GameSessionTestFactory.Create(...)`를 사용한다. public `new GameSessionState(...)`는 `GameSessionRuntimeBootstrapProvider`를 통해 production narrative `Resources` bootstrap을 수행하므로 FastUnit에서 직접 호출하지 않는다.
- authored `ScriptableObject` fixture를 만들지 않는다. test class 본문에서 `ScriptableObject.CreateInstance`, `UnityEngine.Object` lifecycle, `DestroyImmediate`, `using SM.Content.Definitions`, `Resources.Load*`는 금지한다. `FakeCombatContentLookup`은 `ICombatContentLookup` signature 대응을 위해 definition 타입을 보유할 수 있지만 asset 생성과 lifecycle을 소유하지 않는다.
...
- `SM.Editor.Validation`처럼 editor assembly를 직접 검증하는 테스트는 빠르더라도 `BatchOnly`로 둔다.
...
| `FakeCombatContentLookup` | `SM.Tests.FastUnit` | 테스트 — asset API 없이 빈/최소 데이터 반환 |
...
2. **resource/content/session bootstrap outside BatchOnly**: `[Category("BatchOnly")]` 없는 테스트에서 `new RuntimeCombatContentLookup()`, `Resources.LoadAll`, `NarrativeRuntimeBootstrap.LoadFromResources()`, public `new GameSessionState(...)`를 호출하면 실패한다. `Resources` static import/type alias, `RuntimeCombatContentLookup` wrapper/alias, narrative bootstrap wrapper/alias도 실패한다.
3. **ContentConversion ownership boundary**: `Assets/_Game/Scripts/Runtime/Unity/ContentConversion/**`에 로컬 asmdef, public API, session/persistence/UI ownership, registry 밖 asset loading/editor fallback이 들어오면 실패한다.
...
pure asmdef exact reference allowlist, `SM.Tests.FastUnit`의 `SM.Editor`/editor-only package 참조 금지, FastUnit category 전용 폴더 고정, Meta forbidden token, FastUnit authored-object 금지, public session constructor 금지,
`GameSessionState.cs` line budget과 production narrative bootstrap provider choke point를 검사한다. 또한
