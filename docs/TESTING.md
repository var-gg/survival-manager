...
기본 원칙은 모든 EditMode test class가 `FastUnit` 또는 `BatchOnly` 중 하나를 명시하는 것이다. `ManualLoopD`는 장시간 Loop D balance/telemetry 검증용 예외 lane이며, default fast loop나 일반 BatchOnly closure의 대체물이 아니다.
