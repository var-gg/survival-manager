...
- **combat-time provenance** *(1차 구현 — 위 #provenance, session-scoped + readout 노출)*. **남은 것**: match audit/combat event schema에 영속 slot(replay 재검증), 전투 시작/주요 교전 event 세분화. 안 박으면 마지막 felt 닫을 때 modifier application/event log/replay를 다시 관통.
- **incident-centric Dossier**: storage key를 faction이 아니라 incident(Site/Warrant/PromiseType)로 — "네 세력 공동 죄" 캠페인의 교차기록. faction view는 projection.
