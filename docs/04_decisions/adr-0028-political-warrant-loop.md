...
- **(부분 구현) P2b 선택 UI** — 로직 코어 `WarrantOptionBuilder`(제안 warrant + standing → 옵션 + 조건 미리보기[후원/경계], headless 테스트 7) 완료. ID/label 분리: 옵션은 stable id + 미리보기만(표시명 없음). **남은 것(에디터 세션)**: UIToolkit MVP(UXML/USS/View/Presenter/Controller) + `AtlasScreenController.ContinueToExpedition`(`PrepareSelectedBattleNodeHandoff`↔`GoToBattle` 사이) 통합 + 세력/warrant 표시명 localization StringTable(.asset — 코드 lore 금지) + PlayMode visual QA. 상세 plan은 pindoc(analysis-p2-warrant-system-design rev 11).
