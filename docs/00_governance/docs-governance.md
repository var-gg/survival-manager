...
- Pindoc Wiki: 제품 목표, 범위, 타깃 사용자, 게임기획, 창작/narrative/lore, visual design
- `docs/02_design/`: code-facing gameplay/content/UI contract와 Pindoc 전환 hold 문서
...
- Pindoc Decision/Analysis/Glossary/Task: product/design/content/governance soft policy의 기본 durable memory

live 진행 상황을 durable docs에 섞지 않는다.
durable 정책을 task 메모로 대체하지 않는다.
제품/게임기획/창작 방향은 repo Markdown 사본과 Pindoc이 경쟁하지 않게 Pindoc을 우선한다.
...
4. product/design/narrative/lore 방향은 Pindoc context/search를 먼저 사용한다.
5. deprecated 문서, archive, legacy 메모, routing asset은 기본 durable corpus에 섞지 않는다.
...
- deprecated registry 같은 tombstone 문서를 active index 시작면에 직접 노출하지 않는다.
