...
| product / game design planning | `pindoc://decision-doc-harness-pindoc-migration`; Pindoc strategy/experience/mechanisms artifacts | 없음 | 관련 Pindoc Task 또는 현재 repo task `status.md` | `PINDOC.md` | 제품 방향, MVP 범위, design pillars, narrative/lore/visual design은 Pindoc Wiki가 primary SoT다. repo Markdown 사본은 복원해서 active source로 쓰지 않는다. |
| vertical slice live state | Pindoc strategy/mechanisms artifacts; `docs/04_decisions/adr-0014-grid-deployment-continuous-combat.md` | `docs/06_production/index.md`; `docs/06_production/mvp-playtest-checklist.md`; `docs/06_production/pre-art-release-floor.md` | `tasks/001_mvp_vertical_slice/status.md` | 없음 | 현재 플레이어블 경계, 리스크, 다음 우선순위는 task status와 Pindoc artifact를 우선하고, paid asset pass 직전 floor/packet 규칙은 pre-art release floor를 따른다. |
