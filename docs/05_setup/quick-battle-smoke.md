...
1. `SM/저작/전투 샌드박스`로 active handoff Inspector를 연다.
...
6. runtime만 다시 보고 싶을 때는 `SM/전투테스트`를 쓴다.
7. preset 탐색, variant clone, result/history 비교가 필요할 때만 `Window/SM/Combat Sandbox`를 연다.

## persistence 격리 규칙

- `SM/전투테스트` direct entry는 dedicated smoke namespace를 사용한다. 기본 profile이 `default`면 smoke save는 `default.smoke`에 기록된다.
...
| 항목 | 위치 |
| --- | --- |
| team/loadout/tactic/passive/seed/layout 직접 편집 | `SM/저작/전투 샌드박스` -> `CombatSandboxConfig` Inspector |
...
`SM/전체테스트` 메뉴로 normal playable을 띄운 뒤 Town에서도 smoke를 실행할 수 있다.
