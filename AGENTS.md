...
- `FastUnit` 테스트는 `FakeCombatContentLookup`과 `GameSessionTestFactory`를 사용하고, `new RuntimeCombatContentLookup()` 또는 public `new GameSessionState(...)`를 직접 호출하지 않는다.
...
- `RuntimeCombatContentLookup`을 테스트에서 직접 생성하거나 public `GameSessionState` 생성자를 사용하면 `Resources.LoadAll` 경로가 트리거될 수 있다. FastUnit에서는 `FakeCombatContentLookup`과 `GameSessionTestFactory`를 사용한다.
