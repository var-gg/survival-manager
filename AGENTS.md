...
- `FastUnit` 테스트는 editor-free/resource-free/authored-object-free lane이다. `FakeCombatContentLookup`, `GameSessionTestFactory`, pure `CombatContentSnapshot`/spec fixture를 사용하고, `ScriptableObject.CreateInstance`, `SM.Content.Definitions`, `Resources.Load*`, `new RuntimeCombatContentLookup()`, public `new GameSessionState(...)`를 직접 호출하지 않는다.
